...
| ~~**Super-user Control Tower fan-out (no monthly rollup)**~~ | ~~`controlTower` `Promise.all`s `buildTile` over every permitted store, each doing a month-range `.collect()`.~~ | **RESOLVED (#16, commit `<pending>`):** new denormalised `storeMonths` table (mtdNet + latestGpPercent), maintained by `recomputeStoreMonth` inside the Cashup/Gross Profit ingest mutations; `controlTower` now reads one indexed `storeMonths` point-row per store instead of scanning the month's Store Days. `storeMonths.backfill` (scheduler fan-out) rebuilds it; verified on dev. | — done — |
