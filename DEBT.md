...
| ~~**No cursor pagination on Upload History**~~ | ~~`uploads.listForStore` caps the audit trail to the 50 most-recent batches via `.take(50)`.~~ | **RESOLVED (#13, commit `84dc1ec`):** `listForStore` now uses `.paginate()` (50 batches/page); `UploadHistory` panel uses `usePaginatedQuery` + "Load more". Auth-denied callers get an empty exhausted page. | — done — |
